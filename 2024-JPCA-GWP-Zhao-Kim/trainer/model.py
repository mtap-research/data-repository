from data import data_prepare,data_split,normal,normal_tree
import warnings
import joblib
import numpy as np
import pandas as pd
...
def data(data_csv="ipcc_dataset.csv", tar='LF', verbos=False, saveto="dataset4gwp.csv"):
...
def data_tree(data_csv="ipcc_dataset.csv", tar='LF', verbos=False, saveto="dataset4gwp.csv"):
    df_all_features = data_prepare(data_csv,verbos=verbos, saveto=saveto)
    print(df_all_features.head())
    X_train, y_train, X_test, y_test = data_split(data_csv=saveto,ratio = 0.8,verbos = True)
    Xtrain, Ytrain, Xtest, Ytest = normal_tree(X_train, y_train, X_test, y_test,tar=tar)
    return Xtrain, Ytrain, Xtest, Ytest
